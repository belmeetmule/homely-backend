...
RSpec.describe "POST /login", type: :request do
  ...
  let(:user) { User.create!(  full_name: 'full name',
    ...
    describe 'POST /signup', type: :request do
        ...
        before { post '/signup', params: {user: { full_name: 'abebe ho', email: 'extest@gmail.com', password: 'my1234'}}}
        ...
        it 'returns validation errors' do
            expect(response.body).to include("Sorry, user couldn't be created. Email has already been taken")
        end
        end
    end
...
context 'when params are correct' do
  ...
  it 'returns valid JWT token' do
    ...
    # note on decoded_token.first['sub'] The 'sub' claim is a standard claim that is used to identify the subject of the JWT token. The value of the sub claim is typically a unique identifier for the user such as user_id.
    expect(decoded_token.first['sub']).to eq(user.id.to_s) # comparing the user id from the decoded token to the user id from the database
  end
end

context 'when login params are incorrect' do
    #simulate invalid login params
  before { post '/login', params: {user: {email:'invalid@email.com', password: 'invalidpassword'}}, headers: { 'Accept' => 'application/json' }}
